  ...
  <div>
    <BasicTable @register="registerTable" :searchInfo="searchInfo">
      ...
      <template #receiptStatus="{ record }">
        <div v-if="record.receiptStatus == 1">未回执</div>
        <div v-if="record.receiptStatus == 2">已回执</div>
      </template>
...
function drawback1(record: Recordable) {
  drawback({ id: record.id }).catch((err) => {
...
function Approved(record: Recordable) {
  ApprovedPay({
    ...
    if (res.code == 0) {
      createMessage.success("审批退款成功");
    } else {
      createMessage.error("审批退款失败");
    }
    reload();
  });
}
...
function Cancel(record: Recordable) {
  ApprovedPay({
    ...
    if (res.code == 0) {
      createMessage.success(`拒绝退款`);
    } else {
      createMessage.error(`拒绝退款失败`);
    }
    reload();
  });
}
// 上传回执
function  uploadFile(record:Recordable){
  openModal(true, {
    record,
    isUpdate: true,
  });
