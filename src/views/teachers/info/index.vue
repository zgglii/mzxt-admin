<template>
  <div class="teacherInfoView">
    <BasicTable @register="registerTable" :searchInfo="searchInfo" v-if="isEdit">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate">添加老师</a-button>
      </template>
      <template #teacherImg="{ record }">
        <Image :width="40" :src="record.teacherImg" />
      </template>
      ...
      <template #teacherAffirmation="{ record }">
        <div v-html="record.teacherAffirmation" style="max-height: 60px; overflow: auto"></div>
      </template>
...
<script lang="ts" setup>
  ...
  import { teacherList, teacherDel, teacherEdit } from '/@/api/teacherApi/teacherInfo'; // teacher api
  ...
  function handleEdit(record: Recordable) {
    ...
    nextTick(() => {
      ...
      record.teacherImg = [
        {
          url: record.teacherImg,
          response: { data: record.teacherImg },
          uid: 'teacherImg',
          status: 'done',
        },
      ];
  ...
  async function handleSubmit(): void {
    try {
      ...
      let data2 = dataSetRender(values, ['teacherImg']);
      teacherEdit(data2).then(() => {
        isEdit.value = true;
        setProps({
          submitButtonOptions: {
            loading: false,
