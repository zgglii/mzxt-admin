<template>
  <div class="teacherInfoView">
    <BasicTable @register="registerTable" :searchInfo="searchInfo" v-if="isEdit">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate">添加学习内容</a-button>
      </template>
...
<script lang="ts" setup>
  ...
  import { contentList, contentEdit, contentDel } from '/@/api/followme/contents'; // teacher api
  ...
  async function copyTitle(record: Recordable): void {
    if (navigator.clipboard && window.isSecureContext) {
      ...
      return navigator.clipboard.writeText(record.learnTitle);
    } else {
      // 创建text area
      const textArea = document.createElement('textarea');
      textArea.value = record.learnTitle;
  ...
  function handleEdit(record: Recordable) {
    ...
    nextTick(() => {
      ...
      record.contentType = record.contentType.toString();
      record.learnType = record.learnType.toString();
      record.learnIcon = [
        {
          url: record.learnIcon,
          response: { data: record.learnIcon },
          uid: 'learnIcon',
          status: 'done',
        },
      ];
  ...
  async function handleSubmit(): void {
    try {
      ...
      let data2 = dataSetRender(values, ['learnIcon', 'audioUrl']);
      data2.sendTime = moment(data2.sendTime).format('YYYY-MM-DD HH:mm:ss');
      // console.log(
      //   values,
      //   data2,
      //   values.contentType === '0',
      //   values.audioUrl[0].response.data === '',
      //   " console.log(value, '发送请钱参数编辑');",
      // );
      if (values.contentType === '0' && values.audioUrl[0].response.data === '') {
        return createMessage.error('请上传音频文件');
      }
