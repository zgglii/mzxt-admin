<template>
  <div class="teacherInfoView">
    <BasicTable @register="registerTable" :searchInfo="searchInfo" v-if="isEdit">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate">添加调研纪要</a-button>
      </template>
...
<script lang="ts" setup>
  ...
  const thirdCategoryList = [
    { label: '公司纪要', value: 'COMPANY_NOTES' },
    { label: '行业纪要', value: 'INDUSTRY_NOTES' },
    { label: '公司观点', value: 'INSTITUTIONAL_PERSPECTIVE' },
  ];
  ...
  function handleCreate() {
    ...
    isEdit.value = false;
    console.log(getFieldsValue(), 'hhhhhhhhhhhhh');
  }
  // Edit Btn
  let activeID = 0;
  ...
  async function handleSubmit(): void {
    try {
      ...
      data2.reportCategory = 'RESEARCH_NOTES';
