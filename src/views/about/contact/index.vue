...
<script lang="ts" setup>
  ...
  import { usTelList, usTelEdit } from '/@/api/about/contents'; // teacher api
  ...
  function usTelListFN(): records {
    usTelList({
      ...
      let { connectTel, connectService, id } = records[0];
      setFieldsValue({
        connectTel,
        connectService: [
          {
            url: connectService,
            response: { data: connectService },
            uid: 'connectService',
            status: 'done',
          },
        ],
